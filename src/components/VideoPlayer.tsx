    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-0 md:p-6 backdrop-blur-3xl animate-in fade-in duration-200 overflow-hidden">
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-400/40 text-xs font-bold text-cyan-200 flex items-center gap-1.5 transition-all cursor-pointer shadow-md hover:scale-105 active:scale-95 flex-shrink-0"
              title="Switch Streaming Server (11 Servers Available)"
              aria-label="Switch Streaming Server"
            >
              <Server className="w-3.5 h-3.5 text-cyan-400" />
              <span className="hidden sm:inline font-bold text-white">Server:</span>
              <span className="text-cyan-300 font-extrabold">{currentProvider.name}</span>
        ...
        <div className="relative flex-1 w-full h-full bg-[#050508] flex items-center justify-center overflow-hidden">
          {/* Persistent Floating Quick Server Switcher (Always accessible during video playback even when header controls auto-hide) */}
          {activeTab === 'stream' && (
            <button
              type="button"
              onClick={() => {
              ...
              className={`absolute top-3 right-14 sm:top-4 sm:right-20 z-40 px-2.5 py-1.5 rounded-xl bg-black/85 hover:bg-black border border-cyan-500/50 hover:border-cyan-400 text-cyan-300 font-bold text-xs items-center gap-1.5 shadow-2xl backdrop-blur-xl transition-all hover:scale-105 active:scale-95 cursor-pointer select-none ${
                showControls ? 'hidden' : 'flex'
              }`}
              title="Click to Switch Stream Server (11 Servers)"
            >
          ...
          <div className="px-4 py-2.5 bg-black/90 border-t border-white/10 backdrop-blur-xl flex flex-wrap items-center justify-between gap-3 text-xs">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-500/30 text-cyan-300 font-bold text-xs transition-all cursor-pointer shadow-sm active:scale-95"
                title="Click to Switch Stream Server (11 Servers Available)"
              >
                <Server className="w-3.5 h-3.5 text-cyan-400" />
                <span>Server: {currentProvider.name}</span>
                <ChevronDown className="w-3 h-3 text-cyan-400/80" />
              </button>
        ...
        {/* Dedicated Centered Streaming Provider Switcher Modal */}
        {isServerMenuOpen && (
          <div 
            className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200"
            onClick={(e) => {
              if (e.target === e.currentTarget) setIsServerMenuOpen(false);
            }}
          >
            <div className="w-full max-w-md bg-[#0b0c16] border border-cyan-500/40 rounded-3xl shadow-[0_20px_70px_rgba(0,0,0,0.9)] p-4 sm:p-5 space-y-3.5 max-h-[88vh] flex flex-col">
              {/* Header */}
              <div className="flex items-center justify-between pb-3 border-b border-white/10 flex-shrink-0">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl bg-cyan-500/20 border border-cyan-400/40 text-cyan-300 flex items-center justify-center shadow-md shadow-cyan-500/10">
                    <Server className="w-4 h-4" />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-white tracking-tight flex items-center gap-2">
                      <span>Streaming Provider Switcher</span>
                    ...
                    <p className="text-[11px] text-white/50">Switch server engine if current stream is buffering</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              {/* Server List */}
              <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-cyan-500/30">
                {providers.map((p, idx) => {
                  const isSelected = p.id === selectedProviderId;
                  const speedLabel = idx === 0 
                    ? 'Primary 4K/1080p Engine' 
                    : idx === 1 
                    ? 'Multi-Source High Speed' 
                    : idx === 2 
                    ? 'Ultra Fast Mirror' 
                    : idx === 3 
                    ? 'Multi-Audio Stream' 
                    : `Alternative Mirror #${idx + 1}`;
                  return (
                    <button
                      key={p.id}
                      type="button"
                      ...
                      className={`w-full p-2.5 sm:p-3 rounded-2xl border text-xs font-semibold flex items-center justify-between transition-all cursor-pointer ${
                        isSelected
                          ? 'bg-gradient-to-r from-cyan-500/25 to-blue-600/25 border-cyan-400 text-white shadow-lg shadow-cyan-500/15 ring-1 ring-cyan-400/50'
                          : 'bg-white/5 hover:bg-white/10 border-white/10 text-white/80 hover:text-white'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${isSelected ? 'bg-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.9)] animate-pulse' : 'bg-white/20'}`} />
                        <div className="text-left">
                          ...
                          <span className="text-[10px] text-white/40">{speedLabel}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {isSelected ? (
                          ...
                          <span className="text-[10px] text-white/40 group-hover:text-white">Switch</span>
                        )}
                      </div>
                    </button>
                  );
